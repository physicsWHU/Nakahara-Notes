...
>**Lemma 6.1** 如果$\omega\in\Omega^{r}(N)$是闭的形式，则拉回映射的差是恰当的：$$f^{\ast}\omega-g^{\ast}\omega=\marhrm{d}\phi$$，其中$\phi\in\Omega^{r-1}(M)$.

证：将$f$,$g$写为复合映射：$$f=F\cric f_0\qquad\\qquad g=F\cric f$$
其中$f_t:M\to M\times I(p\mapsto(p,t))$在定理6.3已经定义过，则左式可以写为：
$$\left(F \circ f_{0}\right)^{\ast} \omega-\left(F \circ f_{1}\right)^{\ast} \omega =f_{0}^{\ast} \circ F^{\ast} \omega-f_{1}^{\ast} \circ F^{\ast} \omega 
&=-\left\[\mathrm{d} P\left(F^{\ast} \omega\right)+P \mathrm{~d}\left(F^{\ast} \omega\right)\right\]=-\mathrm{d} P F^{\ast} \omega$$
即：
