...
>**Definition (half space)**: $H^n$ is a half space in $\mathbb{R}^n$ 
>$$H^n\equiv \\{(x^1,x^2,\cdots, x^n)\in \mathbb{R}^n: x^n\geq 0\\}$$

We have induced topology for $H^n$ and we can further define the boundary of $H^n$ as
$$\partial H^n\equiv \\{(x^1,x^2,\cdots,x^n)\in \mathbb{R}^n: x^n=0\\}$$
and the interior of $H^n$
$$\text{Int}H^n=H^n-\partial H^n$$

>**Definition (manifold with a boundary)**: 
